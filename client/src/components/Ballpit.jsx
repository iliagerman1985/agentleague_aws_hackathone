...
function TouchStart(e) {
  if (e.touches.length > 0) {
    ...
    for (const [elem, t] of b) {
      ...
      if (D(rect)) {
        // Only prevent default if the topmost element at the touch point is the canvas itself
        const topEl = document.elementFromPoint(A.x, A.y);
        if (!handled && (topEl === elem)) { e.preventDefault(); handled = true; }
        t.touching = true;
        P(t, rect);
        if (!t.hover) {
          t.hover = true;
